...
import { Shield, Camera, Edit3, Loader2, BookText } from "lucide-react";
...
const WriterProfileTab = () => {
  ...
  return (
    <div className="max-w-4xl animate-in fade-in duration-700 pb-20">
      <h2 className="text-4xl lg:text-5xl font-black text-white italic uppercase tracking-tighter mb-12 leading-none">Author <span className="text-[#ff1e6d]">Profile</span></h2>
        ...
        <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
          ...
          <div className="flex-1 text-center md:text-left space-y-6">
             <div>
                ...
                <p className="text-zinc-500 text-xs mt-3 font-medium flex items-center justify-center md:justify-start gap-2 italic">{session?.user?.email}</p>
             </div>

             <Dialog open={open} onOpenChange={setOpen}>
                <DialogTrigger asChild><button className="bg-white text-black px-10 py-4 rounded-[20px] font-black text-[10px] uppercase shadow-xl hover:scale-105 active:scale-95 transition-all">Edit Author Details</button></DialogTrigger>
                <DialogContent className="bg-[#0c0c0e] border-zinc-800 text-white rounded-[35px] p-10 max-w-lg shadow-2xl">
                   ...
                   <form onSubmit={handleUpdate} className="space-y-6 mt-6">
                      ...
                      <button disabled={loading} type="submit" className="w-full bg-[#ff1e6d] hover:bg-[#e61a62] text-white h-16 rounded-2xl font-black text-lg uppercase italic shadow-lg shadow-pink-500/20 active:scale-95 transition-all">{loading ? <Loader2 className="animate-spin mx-auto" /> : "Save Profile"}</button>
                   </form>
                </DialogContent>
             </Dialog>
          </div>
