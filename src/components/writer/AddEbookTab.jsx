      ...
      <form onSubmit={handleUpload} className="space-y-8 bg-[#111113] p-10 lg:p-14 rounded-[50px] border border-white/5 shadow-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
          <div className="flex flex-col space-y-4">
            ...
            <div className="relative h-[70px]"><span className="absolute left-6 top-1/2 -translate-y-1/2 text-[#ff1e6d] font-black text-xl z-10">$</span><Input name="price" type="number" step="0.01" className="w-full h-full pl-12 bg-black/40 border-zinc-800 text-white font-black text-xl rounded-[22px] focus:border-[#ff1e6d]" placeholder="9.99" required /></div>
          </div>
          <div className="flex flex-col space-y-4 ">
            <Label className="text-zinc-100 font-black text-[11px] uppercase tracking-[4px] ml-1">Select Genre</Label>
            <div className="h-[70px]"> 
              <Select onValueChange={setGenre}>
                <SelectTrigger className="w-full h-full bg-black/40 border-zinc-800 text-zinc-100 font-bold rounded-[22px] px-6">
        ...
        <div className="flex flex-col gap-3">
          ...
          {!preview ? (
            <label htmlFor="book-img" className="flex flex-col items-center justify-center w-full h-56 border-2 border-dashed border-zinc-800 rounded-[35px] cursor-pointer hover:border-[#ff1e6d] bg-black/20 group transition-all"><Upload className="text-zinc-600 mb-2 group-hover:scale-110" size={28} /><span className="text-zinc-500 text-[10px] font-black uppercase tracking-[3px]">Select Image File</span><input type="file" id="book-img" className="hidden" accept="image/*" onChange={handleImageChange} /></label>
          ) : (
            <div className="relative h-72 w-full rounded-[35px] overflow-hidden border border-zinc-800 shadow-2xl"><img src={preview} className="w-full h-full object-cover" alt="" /><div className="absolute inset-0 bg-black/40 flex items-center justify-center"><button type="button" onClick={() => { setPreview(null); setSelectedFile(null); }} className="bg-red-600 text-white p-4 rounded-full shadow-2xl hover:scale-110 transition-transform"><X size={24} /></button></div></div>
          )}
        </div>
        ...
        <Button disabled={loading} className="w-full bg-[#ff1e6d] hover:bg-[#e61a62] text-white h-[80px] rounded-[25px] font-black text-2xl shadow-[0_15px_40px_rgba(255,30,109,0.3)] active:scale-95 transition-all uppercase italic leading-none">
