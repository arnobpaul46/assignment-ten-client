...
const BrowseContent = () => {
  ...
  const fetchBooks = async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ search, genre, sort, availability, page, limit: 10 });
  ...
  const handleClearFilters = () => {
    setSearch(""); setGenre("All"); setSort("newest"); setAvailability("all"); setPage(1);
  };

  return (
    <div className="min-h-screen bg-[#09090b] py-16 px-6 lg:px-10 text-white font-sans">
      <div className="max-w-[1400px] mx-auto">
        <h2 className="text-4xl font-black italic mb-10 tracking-tighter leading-none uppercase">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-8">
          {loading ? [...Array(10)].map((_, i) => <EbookSkeleton key={i} />) : books.length === 0 ? (<div className="col-span-full py-24 text-center border-2 border-dashed border-zinc-800 rounded-[40px] bg-zinc-900/10"><p className="text-zinc-600 font-bold italic text-lg tracking-[4px]">No stories found</p></div>) : (
        ...
        {!loading && totalPages > 1 && (
          <div className="flex justify-center items-center gap-6 mt-20">
            ...
            <div className="flex items-center gap-2"><span className="font-black text-xs text-zinc-500">Page</span><span className="font-black text-lg text-[#ff1e6d] italic">{page}</span><span className="font-black text-xs text-zinc-500">of {totalPages}</span></div>
