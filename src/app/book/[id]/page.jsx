...
const BookDetailsPage = () => {
  ...
  const handlePurchase = async () => {
    ...
    if (isRestrictedRole) {
      return toast.error("Authors and Admins cannot purchase ebooks.");
    }
    ...
    try {
      const res = await fetch(`${SERVER_URL}/api/create-checkout-session`, {
        ...
        body: JSON.stringify({ book, userEmail: session.user.email, userName: session.user.name })
      });
      const data = await res.json();
      if (data.url) window.location.href = data.url;
    } catch (err) { toast.error("Failed to open payment gateway"); }
  };
      ...
      <div className="max-w-[85%] mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-16 lg:gap-20 items-start">
          ...
          <div className="flex-1 space-y-8">
             ...
             <div className="pt-6">
                {isPurchased ? (
                  <div className="bg-green-600/10 border border-green-600/30 text-green-500 h-20 px-10 rounded-[25px] flex items-center gap-4 font-black uppercase text-xl italic tracking-tighter">
                     <CheckCircle2 size={30} /> In Your Library
                  </div>
                ) : isRestrictedRole ? (

                  <div className="bg-zinc-900 border border-zinc-800 text-zinc-500 h-20 px-10 rounded-[25px] flex items-center gap-4 font-black uppercase text-sm italic tracking-tighter cursor-not-allowed">
                     <ShieldAlert size={24} /> Only Readers can purchase
                  </div>
                ) : (
                  
                  <button 
                    onClick={handlePurchase}
                    className="h-20 px-12 bg-[#ff1e6d] hover:bg-[#e61a62] text-white rounded-[25px] font-black text-2xl uppercase italic shadow-[0_15px_40px_rgba(255,30,109,0.3)] transition-all active:scale-95 flex items-center gap-4"
                  >
